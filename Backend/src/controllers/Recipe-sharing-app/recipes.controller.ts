...
import type { param, reqQuery } from "../../middlewares/validators/Recipe-sharing-app/index.js";
...
const recipeProjection = {
 ...
 const getSpecificRecipe = asyncHandler(async (req:Request, res:Response, next:NextFunction) => {
  const { id } = req.params as param;
    ...
    const matchingRecipe = await Recipe.aggregate(recipePipeline).exec();
      return res
        .status(200)
        .json(
          new ApiResponse(200, matchingRecipe, "Recipe fetched successfully")
        );
...
//saving favourite recipes
 const saveFavoriteRecipe = asyncHandler(async(req:Request, res:Response) => {
  const { id } = req.params as param;
...
//search based on keywords eg mealtype, tags, ingredients, dietaryPreference,cuisine types
 const searchRecipeByCriteria = async (req:Request, res:Response) => {
  const { q } = req.query as reqQuery;
  ...
  const pipeline = [
    ...
    return res.status(200).json(new ApiResponse(200, "Recipe searched successfully"))
    

};

//get recipe created by user | user id
 const getRecipesCreatedByUser = asyncHandler(async (req:Request, res:Response) => {
  const { user_id } = req.params as param;
    ...
    const userRecipes = await Recipe.aggregate(recipeAggregation);
...
const shareRecipe = asyncHandler(async (req:Request, res:Response, next:NextFunction) => {
  let { id } = req.params as param;

  const isExistingRecipe = await Recipe.findById(id)
  if(!isExistingRecipe) return next(ApiError.notFound(404, "Recipe doesn't exist"))
    ...
    const recipeToShare = await Recipe.aggregate(recipeAggregation);
      return res
        .status(200)
        .json(
          new ApiResponse(200, recipeToShare, "Recipes fetched successfully")
        );
